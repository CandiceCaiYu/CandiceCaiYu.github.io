...
export const terminologyDescription = '以下这些术语将在整个文档中使用，以下是一个快速参考。';
export const terminologyTreeImage = '![组件树](/nextjsDoc/buildingYourApplication/routing/terminology-component-tree.avif)';
export const terminology = [
    '**Tree:** 一种可视化层次结构惯例。比如，组件树有父组件和子组件，文件夹结构等。',
    '**Subtree:** 树的一部分，开始于新根，结束于叶子。',
    '**Root:** 是树或者子树的第一个节点，比如根布局。',
    '**Leaf:** 子树中没有子节点的节点，比如URL路径的最后一段。',
    '![URL segments](/nextjsDoc/buildingYourApplication/routing/terminology-url-anatomy.avif)',
    '**URL Segment:** 通过/分割的URL的一部分。',
    '**URL Path:** 在域名后的URL的一部分（是由段组成的）。',
];
...
export const appRouterContent2 = 'App 路由器在名为app的新目录中运行。app目录和pages目录一起工作，这样可以逐步采用。这样你就可以选择在你的应用程序中一部分用新的app路由，一部分用以前pages路由。如果你的应用程序使用的pages目录，请查看 Pages Router 的文档。';
export const appRouterContent3 = '应用路由器的优先级高于页面路由器。路由跨目录解析为同一URL路径，否则会导致构时建错误， 从而避免发生冲突。';
export const appRouterContentImage = '![app目录结构](/nextjsDoc/buildingYourApplication/routing/next-router-directories.avif)';
export const appRouterContent4 = '默认情况下app内的组件是React服务器组件。这是一项性能优化，可以让你更容易采纳它，你也可以使用客户端组件。';
export const appRouterRecommendationTitle = '推荐:';
export const appRouterRecommendationContent = '如果你是服务器组件的新手，请查看服务器页面。';
...
export const routeSegmentImage = '![路由段](/nextjsDoc/buildingYourApplication/routing/route-segments-to-path-segments.avif)';
...
export const filesConventionsContent3 = '.js, .jsx或者.tsx文件扩展名可应用到特殊文件。';
...
export const componentHierarchyContent2 = [
    'layout.js ',
    'template.js',
    'error.js (ErrorBoundary 组件内)',
    'loading.js (ErrorBoundary 组件内)',
    'not-found.js (ErrorBoundary 组件内)',
    'page.js或者再嵌套一层layout.js ',
];
export const componentHierarchyContent2Image = '![组件层次结构](/nextjsDoc/buildingYourApplication/routing/file-conventions-component-hierarchy.avif)';
export const componentHierarchyContent3 = '在嵌套路由里，段的组件会被嵌套在它父段的组件里。';
export const componentHierarchyContent3Image = '![嵌套路由](/nextjsDoc/buildingYourApplication/routing/nested-file-conventions-component-hierarchy.avif)';


export const colocationTitle = '## 托管';
export const colocationImage = '![托管](/nextjsDoc/buildingYourApplication/routing/project-organization-colocation.avif)';
